...
describe('AnecdoteList', () => {
  ...
  it('increases votes when the vote button is clicked', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      json: vi.fn().mockResolvedValue([])
    })
    ...
    const voteMeLabel = screen.getByText('Vote me')
    const anecdoteItem = voteMeLabel.closest('div')?.parentElement
    expect(anecdoteItem).toBeTruthy()

    const voteButton = within(anecdoteItem).getByText('vote')
    fireEvent.click(voteButton)

    const voteResult = within(anecdoteItem).getByText(/has \d+/)
    expect(voteResult.textContent).toContain('has 1')
  })
})
